...
namespace StoreOnLine.Service.Helpers
{
    ...
    /// <summary>
    /// Esta clase hace varias cosas tales como obtener el HomePage Id, genera los menus, submenus, obtener breakcumb links 
    ...
    public static class HtmlHelpers
    {
        ...
        public static string GetHomePage()
        {
            ...
            if (f.Any())
                homepageid = f.First().Id;

            if (homepageid == -1)
                return "/";

            return string.IsNullOrEmpty(f.First().Url) ? string.Format("/{0}", SeoUrl.GetSeoUrl(homepageid)) : f.First().Url;
        }

        public static int GetFirstPageId()
        {
            var f = from p in db.Pages.Where(p => p.ParentId == -1 && p.IncludeInMenu)
                    orderby p.PageOrder
                    ascending
            ...
            foreach (var r in hm)
            {
                if (Role.IsViewRole(r.ViewRoles))
                {
                    ...
                    if (string.IsNullOrEmpty(r.Url))
                        ul += "<li>" + string.Format("<a href='/{0}'><span class='l'></span><span class='r'></span><span id='{1}' class='t edit-page'>{2}</span></a>", SeoUrl.GetSeoUrl(r.PageId), r.PageId, PropertyString.GetString("Page", r.PageId.ToString()));
                    else
                        ul += "<li>" + string.Format("<a href='{0}'><span class='l'></span><span class='r'></span><span id='{1}' class='t edit-page'>{2}</span></a>", r.Url, r.PageId, PropertyString.GetString("Page", r.PageId.ToString()));
        ...
        private static string SubHeadMenu(int parentId)
        {
            ...
            foreach (var r in shm)
            {
                if (Role.IsViewRole(r.ViewRoles))
                {
                    ul = true;
                    if (string.IsNullOrEmpty(r.Url))
                        li += "<li>" + string.Format("<a href='/{0}' id='{1}' class='edit-page'>{2}</a>", SeoUrl.GetSeoUrl(r.PageId), r.PageId, PropertyString.GetString("Page", r.PageId.ToString()));
                    else
                        li += "<li>" + string.Format("<a href='{0}' id='{1}' class='edit-page'>{2}</a>", r.Url, r.PageId, PropertyString.GetString("Page", r.PageId.ToString()));
                    li += SubHeadMenu(r.PageId);
                    li += "</li>\n\t";
                }
            }
            if (ul)
                ...
                foreach (var r in sm)
                {
                    if (Role.IsViewRole(r.ViewRoles))
                    {
                        if (string.IsNullOrEmpty(r.Url))
                            ...
                            ul += "<li>" + string.Format("<a href='{0}'><span class='l'></span><span class='r'></span><span id='{1}' class='t edit-page'>{2}</span></a>", r.Url, r.PageId, PropertyString.GetString("Page", r.PageId.ToString()));
                        ul += subSideMenu(r.PageId, ParentId);
                        ul += "</li>\n\t";
                    }
                }
        ...
        private static string subSideMenu(int ParentId, int PageId)
        {
            string li = "";
            bool ul = false;
            ...
            foreach (var r in ssm)
            {
                if (Role.IsViewRole(r.ViewRoles))
                {
                    ...
                    if (string.IsNullOrEmpty(r.Url))
                        ...
                        li += "<li>" + string.Format("<a href='{0}' id='{1}' class='edit-page'>{2}</a>", r.Url, r.PageId, PropertyString.GetString("Page", r.PageId.ToString()));
                    li += subSideMenu(r.PageId, PageId);
                    li += "</li>\n\t";
                }
            }
            if (ul)
        ...
        public static HtmlString getUserList(this HtmlHelper helper, string UserName)
        {
            ...
            {
                var u = db.Users.Where(c => c.UserName.Contains(UserName));
                foreach (var r in u)
                {
        ...
        public static HtmlString BreakCumb(this HtmlHelper helper, int pageId)
        {
            return new HtmlString(Gbreakcumb(pageId));
        }

        private static string Gbreakcumb(int pageId)
        {
            Pages p=pageservice.getPageById(pageId);
            bool ShowBreakCum = p.EnableBreakCumb;
            string _breakcumb = String.Format("<span>{0}</a>",PropertyString.GetString("page", pageId.ToString()));
            int ParentId=0;
            do
            {
                ...
                if (page != null)
                {
                    if (page.ParentId == -1)
                    {
                        if(ShowBreakCum)//store root pageid in string and split it on pageview
                            _breakcumb = page.PageId.ToString() + "_" + _breakcumb;
                        else
                            _breakcumb = page.PageId.ToString() + "_";
                ...
                {
                    _breakcumb = page.PageId.ToString();
                    break;
                }
            } while (ParentId != -1);
            return _breakcumb;
        ...
        public static HtmlString GetLanguageSelector(this HtmlHelper helper, bool displayname=false)
        {
            ...
            if(displayname)
                ...
                {
                    currentselected = CultureInfo.CurrentCulture.TwoLetterISOLanguageName == item.LanguageId ? "languageselector languageselected" : "languageselector";
                    html += " <a href='/common/setculture/" + item.LanguageId + "'><span class='"+currentselected+"'><img src='../../Content/Images/flags/" + item.LanguageImageName + "' title='" + item.LanguageName + "' />"+item.LanguageName+"</span></a>";
                }
            else
                foreach (var item in language)
                {
                    currentselected = CultureInfo.CurrentCulture.TwoLetterISOLanguageName == item.LanguageId ? "languageselector languageselected" : "languageselector";
                    html += " <a href='/common/setculture/" + item.LanguageId + "'><span class='" + currentselected + "'><img src='../../Content/Images/flags/" + item.LanguageImageName + "' title='" + item.LanguageName + "' /></span></a>";
                }
